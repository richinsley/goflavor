...
// jl - added expgolomb and varlen symbols
...
// jl - added expgolomb and varlen modifiers
...
unsigned int s_tok2mod(int mod)
{
    ...
    case EXPGOLOMB: return M_EXPGOLOMB;
    case VARLEN:    return M_VARLEN;
    default:
...
int s_mod2tok(unsigned int mod)
{
  switch (mod) {
    ...
    case M_EXPGOLOMB:   return EXPGOLOMB;
    case M_VARLEN:      return VARLEN;
    default:
...
const char *s_mods2stdint(unsigned int mod, const char * t)
{
    static char s[256];
    s[0] = '\0';
    ...
    if(strcmp(t, "int") == 0)
    {
        if (mod == M_NONE) {
            ...
            strcat(s, angelscript() ? "int32" : "int32_t");
            return s;
        }
        ...
        if (mod & M_LONG)
        {
            strcat(s, angelscript() ? "64" : "64_t ");
        }
        else if (mod & M_SHORT)
        {
            strcat(s, angelscript() ? "16" : "16_t ");
        }
        else
        {
            strcat(s, angelscript() ? "32" : "32_t");
        }
    ...
    else if(strcmp(t, "char") == 0)
    {
        if (mod == M_NONE) {
            ...
            strcat(s, angelscript() ? "int8" : "int8_t");
            return s;
        }
    ...
    // some other non-int type
    strcat(s, s_mods2str(mod));
    strcat(s, t);
    return s;
}
...
void s_reset()
{
    s_cxt_top = s_cxt = s_cur_class = NULL;
}
