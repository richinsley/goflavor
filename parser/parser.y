...
/* jl - added modifiers for exponential golomb and google protocol buffers style varlen
/* modifiers */
%token <ival> UNSIGNED SIGNED SHORT LONG CONST ALIGNED STATIC ABSTRACT LITTLE BIG EXPGOLOMB VARLEN
...
%token <vp> VERBATIM_A VERBATIM_GET_A VERBATIM_PUT_A VERBATIM_BOTH_A
...
/* jl - changed free to free_memt for BIT_LITERAL */
primary_expr
    : identifier                { $$ = eval_ident($1); }
    | INT_LITERAL               { $$ = leafi(INT_LITERAL, $1); }
    | DOUBLE_LITERAL            { $$ = leafd(DOUBLE_LITERAL, $1); }
    | BIT_LITERAL               { $$ = leafi(INT_LITERAL, $1->val); free_memt($1); }
