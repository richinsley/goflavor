...
/* language used - jl added angelscript*/
typedef enum {
	L_CPP,
	L_JAVA,
    L_XSD,
...
#define cpp() (lang == L_CPP || lang == L_ANGELSCRIPT)
#define java() (lang == L_JAVA)
#define xsd() (lang == L_XSD)
#define angelscript() (lang == L_ANGELSCRIPT)

// jl - added options for cpp variants
typedef enum {
    L_DCPP, // original flavor cpp code generation
    L_QCPP, // Qt flavor
    L_BCPP  // Boost flavor
} cpp_variant_t;
extern cpp_variant_t cpp_variant;

#define cpp_default() (cpp_variant == L_DCPP)
#define cpp_qt() (cpp_variant == L_QCPP)
#define cpp_boost() (cpp_variant == L_BCPP)
...
extern int current_c_outi_count;
...
/* jl - changed ofp and ofp2 to utilize qfilefacade for file operations */
extern char ofile[], ofile2[], opkg[];
extern void *ofp, *ofp2;
