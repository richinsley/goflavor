...
node_t *eval_array(node_t *expr)
{
    ...
    if (!literal || !(sp->modifiers & M_CONST) || sp->init == NULL) {
        return expr;
    }
    ...
    if (sp->init->op != ARRAY) if (is_literal(sp->init->op)) {
        ret = sp->init;
    } else {
...
eval_array_end:
    if (dim_idx != NULL) free_memt(dim_idx);
    if (dim_values != NULL) free_memt(dim_values);
    return ret;
}
