...
typedef struct _memtracker_t{
    void * memptr;
    struct _memtracker_t * next;
} memtracker_t;

static memtracker_t * memt = NULL;

memtracker_t * memt_tail()
{
    memtracker_t * retv = memt;
    if(!retv)
    {
        return NULL;
    }
...
void free_memt(void * memptr)
{
    memtracker_t * curr = memt;
    memtracker_t * next = NULL;
    if(!memt)
    {
    ...
    else if(memt->memptr == memptr)
    {
        // first entry is the memtracker
        memt = memt->next;
        free(curr->memptr);
        free(curr);
        return;
    ...
    {
        // the given pointer is not tracked
        free(memptr);
    }
}
...
memtracker_t * get_memt(void * memptr)
{
    memtracker_t * retv = memt;
    if(!retv)
    {
    ...
    while(retv != NULL && retv->memptr != memptr)
    {
        retv = retv->next;
    }
...
/* jl - changed from malloc to calloc */
void *getmem(int size)
{
    memtracker_t * tail  = memt_tail();
    memtracker_t * nn = calloc(sizeof(memtracker_t), 1);
    nn->memptr = (void *)calloc(size, 1);
    if (nn->memptr == NULL) fatal("Cannot allocate memory (%d bytes)", size);
...
{
    memtracker_t * mt = get_memt(buf);
    if(!mt)
    {
        fprintf(stderr, "Cannot find tracked memory");
        void *_x = (void *)realloc(buf, size);
        if (_x == NULL) fatal("Cannot allocate memory (%d bytes)", size);
        return _x;
    }
...
char *gethname(char *flname)
{
    ...
    if (len_hname > 3) {
        hname[len_hname-3] = '\0';
        // jl - nope, the extension should be handled in code generation
        // strcat(hname, ".h");
    }
    return hname;
}
