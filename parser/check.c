...
// jl - added check that expgolomb and varlen are only valid for int types
...
unsigned int check_modtype(unsigned int mod, symbol_t *type)
{
    ...
    if (type->ident != INT && type->ident != CHAR) {
        ...
        if((m & M_EXPGOLOMB))
        {
            yyerror("expgolomb meaningless for type '%s'", type->name);
            m &= ~M_EXPGOLOMB;
        }

        if((m & M_VARLEN))
        {
            yyerror("varlen meaningless for type '%s'", type->name);
            m &= ~M_VARLEN;
        }
    }
